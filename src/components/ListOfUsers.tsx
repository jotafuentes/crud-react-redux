...
export function ListOfUsers() {
    ...
    return (
        <>
            <Card className='mx-auto max-w-6xl mt-8'>
                <Title className='text-xl flex gap-4 items-center '>
                    Users
                    <Badge className='border rounded-3xl  px-5 py-1 bg-indigo-200'>{users.length}</Badge>
                </Title>
                <Table className="mt-8 mx-4 flex justify-center">
                    <TableHead>
                        ...
                        {users.map((item) => (
                            <TableRow key={item.name}>
                                ...
                                <TableCell className='flex gap-2 justify-start items-center'>
                                    ...
                                    ' src={`https://unavatar.io/github/${item.github}`} alt={`instagram avatar of ${item.name}`} />
                                    {item.name}</TableCell>
